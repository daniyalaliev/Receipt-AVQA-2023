...
df = df[(df.receipt_currency=='Indonesian rupiah') & (df.question_category == 'count') & (df.split == 'train') & (df.answer <= 3) & (df.answer >=0)]
kick = ['X51007231331', 'X51007231336', 'X51007231338', 'X51007231341', 'X51007231344', 'X51007231346', 'X51006913024']
for ki in kick:
    df.drop(df[df['file_name'] == ki].index, inplace=True)
df.answer = df.answer.astype(int)
encoded_dataset = VisionDataset(df, root, 'train')
dataloader = DataLoader(encoded_dataset, batch_size=16, shuffle=True)
...
#
# model.train()
for i in range(100, num_epochs):
    path = './' + f'count_new/{i}' +'_weights_for_new+weights_indonesian.pth'
    print(path)
    if i % 100 == 0:
    ...
    for idx, batch in tqdm(enumerate(dataloader)):
        ...
        optimizer.zero_grad()
        outputs = model(batch)
        anses = torch.tensor(batch[2], dtype=torch.long)
        # batch.pop('image')
        # batch.pop('answer')
        #outputs = F.softmax(outputs.logits)
        print(outputs, anses)
        loss = F.cross_entropy(outputs, anses.cuda(), class_weights)
        print(loss)
        loss.backward()
