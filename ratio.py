...
class VisionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        question1 = 'What is the total amount of taxes to be paid?'
        question2 = 'What is the total amount?'
        ...
        encoded1 = get_ocr_words_and_boxes(path, question1)
        encoded2= get_ocr_words_and_boxes(path, question2)
        try:
            return (encode_dataset(encoded1), encode_dataset(encoded2)) 
        except:
...
new_df = df[(df.question_type =='ratio') & (df.question=='What is ratio of the amount due and the cash received?')]
...
for idx, batch in tqdm(enumerate(dataloader)):
    ...
    curr = []
    for mem in batch: 
        input_ids = mem["input_ids"].to(device)
        attention_mask = mem["attention_mask"].to(device)
        #token_type_ids = batch["token_type_ids"].to(device)
        bbox = mem["bbox"].to(device)
        image = mem["image"].to(device)
